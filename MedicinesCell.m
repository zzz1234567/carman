...
- (void)stepperValueDidChange:(UIStepper *)stepper {
    NSLog(@"A stepper changed its value from the Medicine Cell: %@.", stepper);
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
        ...
        getPrepCounter = (int)[arrOfPreps count];
...
//            MedicinesCell *curCell = (MedicinesCell *) self;
...
//        }
        ...
        if ([sectionRowToCheck isEqualToString:self.SectionRowHolder.text]) {
            ...
            if (stepper.value == stepper.minimumValue) {
                ...
                // :DELETE
                NSLog(@"DELETE: %@", [array objectAtIndex: rowcombined]);
...
//    self.backgroundColor = [UIColor colorWithRed:0.278 green:0.545 blue:0.855 alpha:0.8]; /*478bda*/
//}
//
