...
#define kApothec @"drugstoresList.plist" // для аптек
#define kUserCredent @"user_mobile_account.plist"

@interface ApothecariesListController (UserAccount)

@end

@implementation ApothecariesListController
...
@synthesize preorderHasSent = _preorderHasSent;
...
-(void)showCustomAlert:(NSString *) title msg:(NSString *)msg actionText:(NSString *)actionText showCancel:(BOOL)showCancel cancelText:(NSString *)cancelText obj2remove:(NSMutableDictionary *)obj {
    ...
    UIAlertAction *alertView = [UIAlertAction
                                ...
                                handler:^(UIAlertAction * action)
                                {
                                    ...
                                    NSMutableArray *array = [[NSMutableArray alloc] initWithArray: [self getContentOfPlist:kApothec]];
                                    [array removeObject:obj];
                                    [array writeToFile:[self dataFilePath:kApothec] atomically:YES];
...
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    NSLog(@"preorderHasSent :%d", self.preorderHasSent);
    
    if (!self.preorderHasSent) {
        [self checkIfApothecaryOnline];
    } else {
        [self checkIfApothecaryProcessing];
    }
...
////        self.preorderHasSent = super.preorderHasSent;
////    }
//    
//    return self;
//}
...
- (NSInteger)tableView:(UITableView *)tableView
 numberOfRowsInSection:(NSInteger)section {
    return (int) [[self getContentOfPlist:kApothec] count];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *simpleTableIdentifier = @"ApothecariesCell";
    int currentRow = (int) indexPath.row;
    NSDictionary *frstLevel = [self.medicinesTNList objectAtIndex:currentRow];
    
    NSLog(@"first levelApothecariesCell: %@", frstLevel);
    
    NSString *textLabelText = @"";
    textLabelText = [NSString stringWithFormat:@"%@", [frstLevel  valueForKey:@"name"]];
    
//    NSString *tempLatinName = [frstLevel valueForKey:@"address"];
//    int lettersCounter = (int) [tempLatinName length];
//    
//    if (lettersCounter > 0) {
//        textLabelText = [NSString stringWithFormat:@"%@ (%@)", [frstLevel valueForKey:@"name"], [frstLevel valueForKey:@"address"]];
//    }
    
    NSString *detailText = @"";
    detailText = [NSString stringWithFormat:@"%@", [frstLevel valueForKey:@"address"]];
    //    int rrow = (int)[self indexFromIndexPath:indexPath currentTbl:tableView];
    
    ApothecariesCell *cell = (ApothecariesCell *)[tableView dequeueReusableCellWithIdentifier:simpleTableIdentifier];
    if (cell == nil) {
        NSArray *nib = [[NSBundle mainBundle] loadNibNamed:simpleTableIdentifier owner:self options:nil];
        cell = (ApothecariesCell *) [nib objectAtIndex:0];
    }
    ...
    Boolean isOnline = [[frstLevel valueForKey:@"online"] boolValue];
    Boolean isProcessing = [[frstLevel valueForKey:@"processing"] boolValue];
    Boolean isDone = [[frstLevel valueForKey:@"done"] boolValue];
    if (isOnline) {
        cell.OnlineHolder.hidden = NO;
        cell.OnlineText.hidden = NO;
        if (isProcessing) {
            cell.ProcessingHolder.hidden = NO;
            cell.ProcessingText.hidden = NO;
            cell.OfflineHolder.hidden = YES;
            cell.OfflineText.hidden = YES;
            cell.OnlineHolder.hidden = YES;
            cell.OnlineText.hidden = YES;
            if (isDone) {
                cell.DoneHolder.hidden = NO;
                cell.DoneText.hidden = NO;
                cell.PresenceText.hidden = NO;
                cell.OfflineHolder.hidden = YES;
                cell.OfflineText.hidden = YES;
                cell.OnlineHolder.hidden = YES;
                cell.OnlineText.hidden = YES;
                cell.ProcessingHolder.hidden = YES;
                cell.ProcessingText.hidden = YES;
                
                // определим НАЛИЧИЕ из данных в plist файле
                NSString *quantityText;
                int requestQuant = [[frstLevel valueForKey:@"quant_request"] intValue];
                int responseQuant = [[frstLevel valueForKey:@"quant_response"] intValue];
                
//                NSLog(@"quantresponse %@ \n", [frstLevel valueForKey:@"quant_response"]);
                
                if (responseQuant > 0) {
                    switch ((int) (responseQuant >= requestQuant)) {
                        case 1:
                            quantityText = [NSString stringWithFormat:@"ВСЕ в наличии"];
                            break;
                        case 0:
                            quantityText = [NSString stringWithFormat:@"%d из %d в наличии", responseQuant, requestQuant];
                            break;
                        default:
                            break;
                    }
                } else {
                    quantityText = [NSString stringWithFormat:@"НЕТ в наличии"];
                }
                
               
                cell.PresenceText.text = quantityText;
                // :END
                
            } else {
                cell.DoneHolder.hidden = YES;
                cell.DoneText.hidden = YES;
                cell.PresenceText.hidden = YES;
            }
        } else {
            cell.ProcessingHolder.hidden = YES;
            cell.ProcessingText.hidden = YES;
        ...
        cell.OfflineHolder.hidden = NO;
        cell.OfflineText.hidden = NO;
//        cell.OnlineHolder.hidden = YES;
//        cell.ProcessingHolder.hidden = YES;
//        cell.DoneHolder.hidden = YES;
    }
    ...
    //
    if (![self plistIsEmpty:kApothec]) {
        NSArray *getData = (NSArray *) [self getContentOfPlist:kApothec];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    int integ = (int) indexPath.row;
    NSLog(@"selected: %d", integ );
}

- (void)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    int integ = (int) indexPath.row;
    NSLog(@"(will) selected: %d", integ );
}
...
- (void)deleteZero_Alerting :(NSIndexPath *)indexPath theCell:(UITableViewCell *)cell obj2remove:(NSMutableDictionary *)obj {
    ...
    ApothecariesCell *theCell = (ApothecariesCell *) cell;
...
-(void) checkIfApothecaryOnline {
    OwnHttpReader *httpObj = [[OwnHttpReader alloc] init];
    httpObj.delegate = self;
    NSString *url = [[NSString alloc] initWithFormat:@"%@", @"http://www.kuznetsov.rlsnet.lan/rlsserv_pricesbyregion.htm"];
    [httpObj checkXML:url tbl:nil];
}

// для выполнения этого пользователь мобильного приложения должен быть зарегистрирован и должен сделать запись в таблицу "preorders"
// @param : email
-(void) checkIfApothecaryProcessing {
    NSArray *userDataFromFile = [self getContentOfPlist:kUserCredent];
    NSString *userEmailFromFile = [userDataFromFile objectAtIndex:0];
    if (userEmailFromFile) {
        OwnHttpReader *httpObj = [[OwnHttpReader alloc] init];
        httpObj.delegate = self;
        NSString *url = [[NSString alloc] initWithFormat:@"http://www.kuznetsov.rlsnet.lan/rlsserv_drugstorepreorder.htm?mobemail=%@", userEmailFromFile];
        [httpObj checkXML:url tbl:nil];
    }
}
...
- (void)returnHTTPResponse:(NSMutableArray *)xmlData {
    ...
    [emptyArray writeToFile:[self dataFilePath:kApothec] atomically:YES];
    if (xmlData) {
        [xmlData writeToFile:[self dataFilePath:kApothec] atomically:YES];
        if ([self plistIsEmpty:kApothec]) {
            [self showAlert:@"Нет выбранных аптек" msg:@"" actionText:@"OK. Перейти к выбору аптек"];
        } else {
            _medicinesTNList = [self getContentOfPlist:kApothec];
            NSLog(@"_medicinesTNList: %@", _medicinesTNList);
        }
        _approvedDeleting = NO;
        [_articleTbl reloadData];
        
        NSLog(@"delegated: %@", xmlData);
    } else {
        NSLog(@"xmldata is empty");
    }
}
...
- (void)childViewController:(ApothecariesCell*)viewCntr
    ...
    NSLog(@"from apothec list: int value: %d ", value);
    NSLog(@"apothecary data: %@", arrayedData);
    NSLog(@"apothecary address: %@", [arrayedData valueForKey:@"address"]);
    
    viewCntr.delegate = self;
    
    // ЗАПРОС на получение данных по предзаказу (позиции)
    
    
//    UIStoryboard *storyboard = [[self.window rootViewController] storyboard];
//    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main"
//                                                             bundle:nil];
//    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:[[NSBundle mainBundle].infoDictionary objectForKey:@"Main"] bundle:[NSBundle mainBundle]];
    AppDelegate *appDel = (AppDelegate *)[UIApplication sharedApplication].delegate;
    UIStoryboard *storyboard = appDel.grabStoryboard;
    PreorderData *preorderView=[storyboard instantiateViewControllerWithIdentifier:@"preorderView"];
    [preorderView.navigationController setNavigationBarHidden:NO animated:NO];
    preorderView.preorderTitleText.text = [arrayedData valueForKey:@"name"]; // данное значение позже будет назначено <newItem.title>    
    preorderView.preorderViewId.text = [NSString stringWithFormat:@"%ld", index];
    preorderView.preorderData = preorderDtls;
    [self.navigationController presentViewController:preorderView animated:NO completion:nil];
}
...
/*
#pragma mark - Отправка запроса в аптеки
// обновление данных, полученных от аптек
// получение общего кол-ва препаратов в наличии и общей суммы 
-(void) updateApothecariesData {
    ...
    if (!userHasEnteredAccount) {
        ...
        NSArray *userDataFromFile = [self getContentOfPlist:kUserCredent];
        NSString *userEmailFromFile = [userDataFromFile objectAtIndex:0];
        NSDictionary *mailDictionary = [NSDictionary dictionaryWithObjectsAndKeys:
                                        userEmailFromFile, @"mobileUser_email", nil];
        NSMutableArray *location_drugstoresQuantTypeData = [[NSMutableArray alloc] initWithObjects:mailDictionary, drugstoresQuantityTypeDictionary, nil];
        ...
        [self readDrugstoresData:jsonData];
