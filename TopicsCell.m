...
- (IBAction)showAlert:(id)sender {
    ...
    NSLog(@".m.lk;,./m,/., from the Topics cell : %@", self.buttonIndexPath );
...
- (void)stepperValueDidChange:(UIStepper *)stepper {
    NSLog(@"A stepper changed its value from the Topics cell: %@.", stepper);
    ...
    NSArray *getFullList = (NSArray *) [self readFullMedicinesListFromPlist];
    ...
    NSString *getLatinTradename = [[[[getFullList objectAtIndex:clickedButtonIndexPath.section] valueForKey:getFirstLetter] objectAtIndex:clickedButtonIndexPath.row] valueForKey:@"latinname"];
    NSString *getForm = [[[[getFullList objectAtIndex:clickedButtonIndexPath.section] valueForKey:getFirstLetter] objectAtIndex:clickedButtonIndexPath.row] valueForKey:@"form"];
    NSString *getNomenid = [[[[getFullList objectAtIndex:clickedButtonIndexPath.section] valueForKey:getFirstLetter] objectAtIndex:clickedButtonIndexPath.row] valueForKey:@"nomenid"];
    NSString *getEancode = [[[[getFullList objectAtIndex:clickedButtonIndexPath.section] valueForKey:getFirstLetter] objectAtIndex:clickedButtonIndexPath.row] valueForKey:@"eancode"];
    NSString *setSectionRow = [NSString stringWithFormat:@"%ld:%ld",(long)clickedButtonIndexPath.section,(long)clickedButtonIndexPath.row] ;
    
    NSLog(@"Tradename: %@", getTradename);
...
//        NSLog(@"before PLus clicked: %d", (int) [arr count]);
//        int getTheCounter = (int) [arr count];
//        if (getTheCounter >= 20) {
//            // prevent from updaitng
//            // show an alert
////            self.defaultStepperLabel.text = [NSString stringWithFormat:@"%ld", (long) 0];            
////            self.quantity.text = [NSString stringWithFormat:@"%ld", (long) 0];
//        }
    }
...
//    NSLog(@"array counter: %d", (int) [array count]);
    
    BOOL theRowSectionArePresent = NO;
    NSMutableArray *objectsToRemove = [[NSMutableArray alloc] init];
    for (NSMutableDictionary *dataForRow in array) {
        // row/section есть данные в файле
        if ([[dataForRow objectForKey:@"section"] intValue] == [[NSNumber numberWithInteger:clickedButtonIndexPath.section] intValue] &&
            [[dataForRow objectForKey:@"row"] intValue] == [[NSNumber numberWithInteger:clickedButtonIndexPath.row] intValue]) {
    ...
    if (!theRowSectionArePresent) {
        ...
        if (stepperLabel.text != nil) {
            ...
            [newDictionary setObject:@"" forKey:@"quantity"];
        }
        
        [newDictionary setObject:getTradename forKey:@"prepname"];
        [newDictionary setObject:getLatinTradename forKey:@"latinname"];
        [newDictionary setObject:getForm forKey:@"form"];
        [newDictionary setObject:getNomenid forKey:@"nomenid"];
        [newDictionary setObject:getEancode forKey:@"eancode"];
        [newDictionary setObject:setSectionRow forKey:@"sectionrow"];
...
//    TopicsCell *tcell = (TopicsCell *) [[self getTableView] cellForRowAtIndexPath:path];
//    
